...
class VolRepository extends ServiceEntityRepository
{
     ...
     * @throws OptimisticLockException
     */
    public function decrementerNbSieges(Vol $entity, bool $flush = true): void
    {
        $nbSieges=$entity->getNbsieges();
        $entity->setNbsieges($nbSieges-1);
        $this->_em->persist($entity);
        if ($flush) {
            $this->_em->flush();
        }
     ...
     * @throws OptimisticLockException
     */
    public function incrementerNbSieges(Vol $entity, bool $flush = true): void
    {
        $nbSieges=$entity->getNbsieges();
        $entity->setNbsieges($nbSieges+1);
        $this->_em->persist($entity);
